...
function createQuestion($school_year_id, $question, $criteria_id)
{
  ...
  try {
    ...
    $csql = "SELECT * FROM tblquestion WHERE question =?";
    $stmtc = $conn->prepare($csql);
    $stmtc->bind_param("s", $question);
    $stmtc->execute();
    $stmtc->store_result();
